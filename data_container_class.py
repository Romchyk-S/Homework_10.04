...
class DataContainer:
    ...
    def plot(self, fig=None, ax=None, title = "My preferred title", text="ОВ", **kwargs):
        ...
        if None not in inflexion_points.values():
            
            x_infl, y_infl = inflexion_points.get('x'), inflexion_points.get('y')
            
            if type(x_infl) == int or type(y_infl) == float:
                
                ax.scatter(x_infl, y_infl)
    
                ax.annotate(f'inflexion: x = {x_infl}', xy=(x_infl, y_infl), xytext=(x_infl, y_infl-(abs(y_lim_bottom)+add_step)/2),
                    arrowprops = dict(facecolor = 'black', shrink=1.00, headwidth = 3.5, width = 0.1))
                
            else:
            
                for x_val, y_val in zip(x_infl, y_infl):
                    
                    ax.scatter(x_val, y_val)
        
                    ax.annotate(f'inflexion: x = {x_val}', xy=(x_val, y_val), xytext=(x_val, y_val-(abs(y_lim_bottom)+add_step)/2),
                        arrowprops = dict(facecolor = 'black', shrink=1.00, headwidth = 3.5, width = 0.1))
